<!-- 
  ? PLAN
  ? Change handler for table, row, column, cell
  ? Possible events for table, row, column, cell

  TODO
  * Refactor table into smaller components: table-row, table-head, table-column, table-cell
...
<template>
  <ClientOnly>
    <div v-if="props.dev" id="dev-panel" class="card bg-secondary">
      ...
      <Json :data="rows" title="Rows" />
      <Json :data="newItem" title="newItem" />
      <p>{{ states.isShownRows }}</p>
      ...
      <template #default>
        <div class="p-3 table-wrapper" :class="itemName">
          ...
          <table :class="'table-' + itemName">
            <TableHead :columns="columns" :suggestions="ref([])" />
            ...
            <table-body v-if="rows.length" v-show="states.isShownRows" />
          </table>
        </div>
      </template>
      <template #fallback>
...
const props = defineProps({
  ...
  events: {},
  allRows: {},
  isDefaultShowRows: { default: true, type: Boolean },
  showSuggestions: {
    default: true,
  },
  tableId: { default: "" },
...
let rows = ref([]);
// await nextTick();
if (props.rows) rows = inject(props.rows, []);
if (props.dbTable)
...
watch(
  ...
  (newValue, oldValue) => {
    if (oldValue === true && newValue === false) {
      upsertRow(states.rowBeingEdited);
    }
  },
);
let previous = null;
        ...
        h("input", {
          ...
          class: [
            element.key,
            {
              hidden:
                item.state.isSelected === null ||
                !(selectedRows.value[0] || states.isShownSelection),
            },
          ],
...
async function deleteRows(id) {
  ...
  dbDelete(itemName, deleteRowList);
}
</script>
