...
const propSuggestions = props.suggestion || ref([]);
...
console.log(propSuggestions.value);
const suggestionItems = computed(() => {
  if (!newItem.name) return null;
  //Filter all rows that resemble new item input
  const allSuggestions = propSuggestions.filter((name) =>
    name.toLowerCase().startsWith(newItem.name.toLowerCase()),
  );
          ...
          case "input-name":
            ...
            watch(
              ...
              (input) => {
                ...
                const match = props.suggestion.find((suggestion) =>
